...
class DIEN_CB(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.use_item_id = not config.get('no_itemid', False)  # 如果使用内容特征，则不使用item_id
        ...
        self.types = ["user", "item"] if self.use_user_features else ["item"]  # 条件性包含user类型
        self.user_feat = dataset.get_user_feature() if self.use_user_features else None
        ...
        # 计算user特征维度 - 修复这里的bug
        if self.use_user_features and self.use_item_id:
