...
class DIN_CB(SequentialRecommender):
    ...
    def get_item_representations(self, user, item_seq_next_item):
        ...
        if self.use_item_id:
            ...
            traditional_item_features = feature_table["item"]  # [batch_size, max_length, traditional_dim]
        else:
            traditional_item_features = None

        # 获取音频特征
        if self.use_audio:
            # 提取所有items（包括序列和target）
            all_items = item_seq_next_item.view(-1)  # [batch_size * max_length]
                ...
                if traditional_item_features.size(-1) == audio_item_features.size(-1):
                    ...
                    if not hasattr(self, 'audio_proj'):
                        self.audio_proj = nn.Linear(
                            audio_item_features.size(-1), 
                            traditional_item_features.size(-1)
                        ).to(self.device)
