...
class ContextRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.token2id = dataset.field2token_id
        self.id2token = {}
        save_path = os.path.join(config['data_path'], f"results/{self.config['model']}/{self.config['task_name']}")

        os.makedirs(save_path, exist_ok=True)
        with open(f'{save_path}/id2token.pkl', 'wb') as fp:
            pickle.dump(self.token2id, fp)
