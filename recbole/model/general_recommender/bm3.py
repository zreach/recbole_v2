...
class BM3(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        if self.a_feats is not None:
            ...
            nn.init.xavier_normal_(self.audio_trs.weight)
    ...
    def calculate_loss(self, interactions):
        ...
        t_feat_online, a_feats_online = None, None
        if self.t_feat is not None:
            t_feat_online = self.text_trs(self.text_embedding.weight)
        if self.a_feats is not None:
            a_feats_online = self.audio_trs(self.audio_embedding.weight)
        ...
        with torch.no_grad():
            ...
            if self.a_feats is not None:
                a_feats_target = a_feats_online.clone()
                a_feats_target = F.dropout(a_feats_target, self.dropout)
        ...
        if self.a_feats is not None:
            a_feats_online = self.predictor(a_feats_online)
            a_feats_online = a_feats_online[items, :]
            a_feats_target = a_feats_target[items, :]
            loss_v = 1 - cosine_similarity(a_feats_online, i_target.detach(), dim=-1).mean()
            loss_vt = 1 - cosine_similarity(a_feats_online, a_feats_target.detach(), dim=-1).mean()
