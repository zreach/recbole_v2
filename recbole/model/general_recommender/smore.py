...
class SMORE(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        audio_adj_file = os.path.join(dataset_path, 'audio_adj_{}_{}.pt'.format(self.audio_knn_k, self.sparse))
        ...
        if self.a_feats is not None:
            self.audio_embedding = nn.Embedding.from_pretrained(self.a_feats, freeze=False)
            if os.path.exists(audio_adj_file):
                ...
                audio_adj = build_knn_normalized_graph(audio_adj, topk=self.audio_knn_k, is_sparse=self.sparse,
                                                       norm_type='sym')
        ...
        if self.a_feats is not None:
            self.audio_trs = nn.Linear(self.a_feats.shape[1], self.embedding_dim)
        # if self.t_feat is not None:
        ...
        self.audio_complex_weight = nn.Parameter(torch.randn(1, self.embedding_dim // 2 + 1, 2, dtype=torch.float32))
    ...
    def max_pool_fusion(self):
        audio_adj = self.audio_original_adj.coalesce()
        text_adj = self.text_original_adj.coalesce()

        audio_indices = audio_adj.indices().to(self.device)
        audio_values = audio_adj.values().to(self.device)
        text_indices = text_adj.indices().to(self.device)
        text_values = text_adj.values().to(self.device)

        combined_indices = torch.cat((audio_indices, text_indices), dim=1)
        combined_indices, unique_idx = torch.unique(combined_indices, dim=1, return_inverse=True)

        combined_values_audio = torch.full((combined_indices.size(1),), float('-inf')).to(self.device)
        combined_values_text = torch.full((combined_indices.size(1),), float('-inf')).to(self.device)

        combined_values_audio[unique_idx[:audio_indices.size(1)]] = audio_values
        combined_values_text[unique_idx[audio_indices.size(1):]] = text_values
        combined_values, _ = torch.max(torch.stack((combined_values_audio, combined_values_text)), dim=0)

        fusion_adj = torch.sparse.FloatTensor(combined_indices, combined_values, audio_adj.size()).coalesce()
    ...
    def spectrum_convolution(self, audio_embeds, text_embeds):
        """
        Modality Denoising & Cross-Modality Fusion
        """
        audio_fft = torch.fft.rfft(audio_embeds, dim=1, norm='ortho')           
        text_fft = torch.fft.rfft(text_embeds, dim=1, norm='ortho')

        audio_complex_weight = torch.view_as_complex(self.audio_complex_weight)   
        ...
        #   Uni-modal Denoising
        audio_conv = torch.fft.irfft(audio_fft * audio_complex_weight, n=audio_embeds.shape[1], dim=1, norm='ortho')    
        text_conv = torch.fft.irfft(text_fft * text_complex_weight, n=text_embeds.shape[1], dim=1, norm='ortho')

        #   Cross-modality fusion
        fusion_conv = torch.fft.irfft(text_fft * audio_fft * fusion_complex_weight, n=text_embeds.shape[1], dim=1, norm='ortho') 
        
        return audio_conv, text_conv, fusion_conv
    ...
    def forward(self, adj, train=False):
        ...
        audio_conv, text_conv, fusion_conv = self.spectrum_convolution(audio_feats, text_feats)
        audio_item_embeds = torch.multiply(self.item_id_embedding.weight, self.gate_v(audio_conv))
        ...
        agg_audio_embeds = fusion_soft_v * audio_embeds
        ...
        audio_prefer, text_prefer, fusion_prefer = self.dropout(audio_prefer), self.dropout(text_prefer), self.dropout(fusion_prefer)
        ...
        side_embeds = torch.mean(torch.stack([agg_audio_embeds, agg_text_embeds, fusion_embeds]), dim=0) 
