            ...
            while len(check_list) > 0:
                ...
                check_list = np.array(
                    [
                        ...
                        for i, used, v in zip(
                            check_list,
                            self.used_ids[key_ids[check_list]], #这个就是每个用户已经看过的表
...
class Sampler(AbstractSampler):
    ...
    def set_summary(self):
        # self.user__inter_num = 
        inter_feat = self.datasets.inter_feat
        self.interaction_summary = inter_feat.groupby('user_id:token')['num_repeat:float'].sum().reset_index()
