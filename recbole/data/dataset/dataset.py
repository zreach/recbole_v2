...
class Dataset(torch.utils.data.Dataset):
    ...
    def _data_processing(self):
        ...
        self._normalize()
        self._discretization() # 这一步把连续数据离散化
    ...
    def _set_label_by_threshold(self):
        ...
        counts = self.inter_feat[self.label_field].value_counts()

        # 计算比例
        total = counts.sum()
        proportion_1 = counts.get(1, 0) / total  # 防止没有1的情况

        self.logger.info(f"正负数据比例:{(proportion_1 / (1 - proportion_1)):.2f} : 1")
    ...
    def _concat_remaped_tokens(self, remap_list):
        ...
        for feat, field, ftype in remap_list: # 第一个是交互，第二个是user or item
            # import pdb; pdb.set_trace()
            if ftype == FeatureType.TOKEN:
    ...
    def _remap(self, remap_list):
        ...
        for (feat, field, ftype), new_ids in zip(remap_list, new_ids_list):
            if field not in self.field2id_token:
                ...
                self.field2token_id[field] = token_id #记录每个之前ID和映射后的ID的对应
            if ftype == FeatureType.TOKEN:
                feat[field] = new_ids
            elif ftype == FeatureType.TOKEN_SEQ:
                split_point = np.cumsum(feat[field].agg(len))[:-1]
