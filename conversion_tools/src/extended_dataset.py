...
class Music4All(BaseDataset):
    def __init__(self, input_path, output_path, interaction_type, feature_name, float_seq=True, binary=False):
        super(Music4All, self).__init__(input_path, output_path)
        self.dataset_name = 'm4a'
        self.sep = '\t'
        self.float_seq = False
        self.binary = binary
        ...
        if self.interaction_type == 'counts':
            ...
            self.inter_file = os.path.join(self.input_path, 'listening_history.csv')
            # selected feature fields
            self.inter_fields = {0: 'user_id:token',
                                 1: 'track_id:token',
                                 2: 'timestamp:float',
        ...
        if feature_name != 'none':
            ...
            feature_filename = 'id_' + feature_name + '.csv'
        ...
        if self.interaction_type == 'timestamp':
            with open(self.inter_file, 'r') as f:
                ...
                while True:
                    ...
                    d = datetime.strptime(count, "%Y-%m-%d %H:%M")
